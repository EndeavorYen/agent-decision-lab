...
function render(state){latest=state;document.querySelector('#live').classList.add('live');document.querySelector('#liveText').textContent='live';const title=state.initialized?state.experiment.title:'No lab initialized';document.querySelector('#title').textContent=title;document.querySelector('#topTitle').textContent=title;document.querySelector('#tree').textContent=state.treeText||state.error||'Initialize a case study to see the tree.';renderMetrics(state);renderDoctor(state.doctor);renderRouteRail(state);renderGraph(state);renderEvents(state.events||[]);selectRoute(preferredRoute(state));}
